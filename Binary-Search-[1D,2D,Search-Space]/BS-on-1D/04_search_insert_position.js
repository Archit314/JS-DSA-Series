class Solution {
    ...
    findIndex(low, high, inputArray, element){
        ...
        if(element > inputArray[mid]){
            return this.findIndex(mid+1, high, inputArray, element)
        }
        else{

            // Current element may be the answer,
            // but there could be a smaller valid index on the left
            return this.findIndex(low, mid-1, inputArray, element)
        }
    }
}
    ...
    if(inputData.length === 2){
        ...
        if(enteredArray.length > 0){
            ...
            if(!isNaN(searchElement)){
                ...
                console.log(`Element to insert: ${searchElement}`)

                // Find and print the Upper bound index
                console.log(`${searchElement} can be insert at index: ${new Solution(enteredArray).findIndex(0, enteredArray.length-1, enteredArray, searchElement)}`)
                return
            }
            else{
